...
class TooltipckFields {
	...
	function __construct() {
		// if (is_admin()) {
			if (isset($_GET['page']) && $_GET['page'] === 'tooltip-ck') add_action('admin_init', array( $this, 'load_assets_files'));
		// }
	}
	...
	public function load_assets_files() {
		...
		wp_enqueue_script('tooltip-ck-color-picker', $this->pluginurl . '/fields/color.js', array('jquery', 'jquery-ui-button', 'wp-color-picker'));
		// for the media manager
		wp_enqueue_media();
		wp_enqueue_script('tooltip-ck-media', $this->pluginurl . '/fields/media.js', array('jquery'));
		// for radio buttons
		wp_enqueue_style('tooltip-ck-radio', $this->pluginurl . '/fields/radio.css');
	}

	public function render($type, $name, $classname = '', $optionsgroup = '', $isfiles = false, $attribs = '') {
		$function = 'get'.ucfirst($type);
		$this->name = $this->getName($name);
		$this->id = $this->getId($name);
		$this->value = $this->getValue($name);
	...
	private function getRadio() {
		...
		foreach ($options as $value => $name) {
			...
			$html[] = '<input type="radio" id="' . $this->id . $value . '" name="' . $this->name . '" value="' . htmlspecialchars($value, ENT_COMPAT, 'UTF-8') . '"' . $checked . $class . ' />';
			$html[] = '<label for="' . $this->id . $value . '" class="ckbutton">' . __($name, 'tooltip-ck') . '</label>';
		}
